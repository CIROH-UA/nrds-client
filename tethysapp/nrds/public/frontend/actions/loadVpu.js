 ...
 * lands. When a feature is selected (a cross-vpu click that landed here), the load's closing step
 * charts it through loadTimeseries under the same generation, so the plot for the new vpu appears
 * without a second round trip; on startup nothing is selected, so this simply clears the loading text.
 */
...
export async function loadVpu() {
  ...
  try {
    ...
    const featureId = store.get().feature.selected_feature?._id ?? null;
    if (featureId) {
