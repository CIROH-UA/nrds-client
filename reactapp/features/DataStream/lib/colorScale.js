...
// A class or rank scale reads the value distribution, not just its bounds, so it samples the flat
// array the way computeBounds does: strided, sentinel-free, and capped so the sort stays cheap.
...
// The class-scale samples and breaks, remembered per (variable, scale) and refreshed when the
// underlying value array is replaced, so a scale change never re-sorts the same distribution.
